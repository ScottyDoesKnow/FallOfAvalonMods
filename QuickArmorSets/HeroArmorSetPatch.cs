...
{
    [HarmonyPatch(typeof(HeroArmorSet), nameof(HeroArmorSet.ValidateSetOnSlotChanged))]
    [HarmonyPrefix]
    public static bool ValidateSetOnSlotChangedPrefix(EquipmentSlotType slot)
    {
        try
        {
            Plugin.LogDebug($"{nameof(HeroArmorSetPatch)}.{nameof(ValidateSetOnSlotChangedPrefix)} | Called");
            return VCQuickArmorSet.ArmorSlotTypes.Contains(slot);
        }
        catch (Exception ex)
        {
            Plugin.LogError($"{nameof(HeroArmorSetPatch)}.{nameof(ValidateSetOnSlotChangedPrefix)} | Error: {ex}");
